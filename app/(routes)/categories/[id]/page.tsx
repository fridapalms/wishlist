...
export const WishesByCategory = async ({ params }: WishesByCategoryProps) => {
  ...
  if (wishes.length === 0) {
    return (
      <div className="flex flex-col gap-8 md:p-6 p-2">
        <PageNavigation items={[{ label: "Home", href: "/" }, { label: "Categories", href: "/categories" }, { label: category.name }]} />
        <h1 className="text-3xl text-start font-semibold text-[var(--color-darkgreen)]">{category.name}</h1>
        <h3 className="text-2xl text-start font-semibold text-[var(--color-darkgreen)]">Oops, this category is empty.</h3>
